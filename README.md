...
- dynamic_whitelist 动态 DDNS 来源白名单
- GeoIP / CN IP 来源限制
...
- access_control / dynamic_whitelist / GeoIP 限制“谁能访问入口”
...
v0.8.0 新增 **动态 DDNS 来源白名单**（`dynamic_whitelist`）。该功能属于 `access_control` 的来源 IP 白名单增强：用户把自己的家宽 / 手机热点 / 公司出口 IP 更新到已有 DDNS 域名，本项目定期解析这些 DDNS，并把解析到的 IP 并入来源白名单。

边界保持不变：

- 只影响本项目管理的转发规则来源限制
- 不影响 `egress_control.allowed_target_cidrs`
- 不影响目标域名 / 目标 DDNS / 目标 last-good 解析
- 不影响 SSH GeoIP 限制
- 不引入 WebUI / nat-console / 多用户架构 / 数据库存储 / DNS 供应商接口
- 第一版默认 IPv4，IPv6 仅在 `resolve_ipv6=true` 且规则侧已有 IPv6 来源限制能力时使用
...
`access_control`（黑/白名单）、`dynamic_whitelist`（动态 DDNS 来源白名单）与 `geoip`（国家/地区限制）是分层叠加的访问控制，不是互相替代。多个来源限制同时启用时，使用 AND 逻辑：
...
2. **白名单是精确来源限制。** `access_control.mode = "whitelist"` 时，只有 `entries` 内的静态来源 IP/CIDR 或 `dynamic_whitelist` 当前解析出的来源 IP 能匹配本项目转发规则，其他来源不会被 DNAT，等同于拒绝转发；不会因为属于 CN/LAN 而被放行。
...
- 白名单开启且来源不在静态白名单 / dynamic_whitelist：即使属于 CN 也不会被转发。
...
`dynamic_whitelist` 用于自己的家宽 / 手机热点 / 公司出口 IP 经常变化的场景。用户先在自己的 DDNS 服务中维护域名解析，本项目只定期解析这些 DDNS 域名，把解析到的 IP 加入来源白名单。

它是什么：

- `access_control` 的来源 IP 白名单增强
- 限制“谁能访问入口”
- `access_control.mode = "whitelist"` 时才参与来源放行
- DNS 成功时当前解析到哪些 IP，就允许哪些 IP
- DNS 失败时可用 last-good 来源 IP 临时兜底
...
- state 文件默认 `/var/lib/nftables-nat-rust/dynamic-whitelist-state.json`，独立于目标 last-good state
- state 文件损坏会 WARN 并尝试重新生成；写入失败只 WARN + audit，不让 nat.service 崩溃
...
- 只使用你自己控制的 DDNS
- 如果 DDNS 账号被盗，攻击者可能把恶意 IP 加入白名单
- 建议保留至少一个静态白名单 IP 作为兜底
- 不要把 dynamic whitelist 当成公开访问授权机制
...
`snat.mode` 控制 POSTROUTING 阶段的源地址改写方式。普通 VPS 端口转发一般使用默认值 `masquerade`，私有网络出口机或落地机场景可使用 `fixed` 指定固定中转源 IP。

- `mode = "masquerade"`（默认推荐）：生成 `masquerade` 规则，由 nft 自动选择出接口源 IP，适合普通 VPS
- `mode = "fixed"`：生成 `snat to <fixed_source_ip>`，适合私有网络出口机需要让落地机看到固定中转内网 IP 的场景；**第一版仅支持 IPv4**
...
`mss_clamp` 在本项目转发链路上对 TCP SYN 包写入 `tcp option maxseg size set <size>`，缓解多跳 / 隧道 / 私有网络链路 / MTU 异常场景下的测速异常、网页卡顿、TLS 握手卡死等问题。
...
- 黑名单优先级最高，命中即拒绝
- 白名单是精确来源 IP 限制，`dynamic_whitelist` 是动态来源白名单
...
- `access_control.update` / `dynamic_whitelist.config.update` / `geoip.update` / `egress_control.update` / `snat.update` / `mss_clamp.update`
...
- `dynamic_whitelist.resolve.success` / `dynamic_whitelist.resolve.fail` / `dynamic_whitelist.change` / `dynamic_whitelist.prune`
...
- 添加 / 删除 / 启用 / 禁用规则
- access_control（白名单 / 黑名单）
- dynamic_whitelist 域名添加 / 删除 / 启用 / 禁用
...
- `main.rs`：入口、CLI 参数解析、顶层流程；`handle_loop` / `refresh_once` / `RuntimeConfig` / `parse_conf` / `build_new_script` / dynamic_whitelist 刷新与合并等仍保留在这里
...
- `dynamic_whitelist.rs`：动态 DDNS 来源白名单 state、mockable 解析刷新、last-good 来源 IP 兜底
- `last_good.rs` / `quota.rs` / `stats.rs` / `geoip.rs` / `forward_test.rs` / `uninstall.rs` / `logger.rs`：保持各自原职责

非 Rust 部分：

- `install.sh` / `setup.sh` / `tests/install-dry-run.sh`：仍为 core-only 安装 / setup / dry-run
- `.github/workflows/`：release 构建
