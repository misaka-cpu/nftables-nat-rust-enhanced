...
CLI 主菜单 `20) 查看审计日志` 默认显示最近 50 行，并提示 audit 文件路径。完整日志可在配置的 `audit.file` 路径用 `tail -F` / `grep` 直接查看。
...
`高级网络设置` 子菜单：查看 SNAT / MSS 状态、设置 SNAT 模式（masquerade / fixed / off）、设置 fixed SNAT 源 IP、启用 / 禁用 MSS clamp、设置 MSS clamp size。

`查看审计日志` 显示最近 50 行 audit JSON，默认 `audit.file = /var/log/nftables-nat-rust-audit.log`。完整文件可用 `tail -F` / `grep` 直接查看。
